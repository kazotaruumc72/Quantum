 ...
 * Usage: /exp <player> <give|take|set|reset> <amount>
 */
public class ExpCommand implements CommandExecutor {
    ...
    public ExpCommand(Quantum plugin, PlayerLevelManager levelManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage("§6§l[EXP] §7Usage: §f/exp <joueur> <give|take|set|reset> [montant]");
            return true;
        }
        ...
        switch (action) {
            case "give":
                if (args.length < 3) {
                    sender.sendMessage("§c§l✗ §cUsage: §f/exp <joueur> give <montant>");
                    return true;
                }
                try {
            ...
            case "take":
                if (args.length < 3) {
                    sender.sendMessage("§c§l✗ §cUsage: §f/exp <joueur> take <montant>");
                    return true;
                }
                try {
                    int amount = Integer.parseInt(args[2]);
            ...
            case "set":
                if (args.length < 3) {
                    sender.sendMessage("§c§l✗ §cUsage: §f/exp <joueur> set <montant>");
                    return true;
                }
                try {
                    int amount = Integer.parseInt(args[2]);
